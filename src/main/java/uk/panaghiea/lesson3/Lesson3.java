...
public class Lesson3 {
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        if (parallel) {
            return calculateLevenshteinParralel(wordList, LIST_SIZE);
        } else {
            return calculateLevenshtein(wordList, LIST_SIZE);
        }
    }

    private static int[][] calculateLevenshtein(List<String> wordList, int wordListSize) {
        int[][] distances = new int[wordListSize][wordListSize];
        IntStream.range(0, wordListSize).forEach(i -> IntStream.range(0, i).forEach(j ->
                        calculateDistance(wordList, distances, i, j)
        ));
        return distances;
    }

    private static int[][] calculateLevenshteinParralel(List<String> wordList, int wordListSize) {
        int[][] distances = new int[wordListSize][wordListSize];
        IntStream.range(0, wordListSize).parallel().forEach(i -> IntStream.range(0, i).parallel().forEach(j ->
                        calculateDistance(wordList, distances, i, j)
        ));
        return distances;
    }

    private static void calculateDistance(List<String> wordList, int[][] distances, int i, int j) {
    ...
    static List<String> processWords(List<String> wordList, boolean parallel) {
        ...
        if (parallel) {
            wordListStream = wordListStream.parallel();
        }
        return wordListStream.sorted((s1, s2) -> s1.length() - s2.length()).map(String::toUpperCase).collect(Collectors.toList());
    }
    ...
    public static void main(String[] args) throws IOException {
        RandomWords fullWordList = new RandomWords(Files.newBufferedReader(Paths.get("words.txt")));
        List<String> wordList = fullWordList.createList(2000);
